...
- Семафор на 1 параллельный пайплайн; остальные ждут в FIFO-очереди (`pipeline_queue`)
  и видят свою позицию в статусном сообщении. Больше `MAX_QUEUE_LENGTH` (5) ожидающих —
...
- Лимиты входа: `MAX_AUDIO_DURATION_SECONDS` (4 ч) и `MAX_AUDIO_BYTES` (500 МБ) проверяются
  по метаданным Telegram до скачивания
